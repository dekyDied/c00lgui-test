--[[
    ...
    local Window = UI:CreateWindow({
        Title    = "My Script",
        SubTitle = "v1.0.0",
        Icon     = "layout-grid", -- top-left logo icon, Lucide name or rbxassetid
...
function Library:CreateWindow(config)
    ...
    create("Frame", {
        ...
        corner(6),
        create("ImageLabel", {
            Image = resolveIcon(config.Icon or "layout-grid") or "",
    ...
    -- Ghost icon button helper (search / minimize / expand / close style)
    local function ghostIconButton(iconName, xOffset, size)
        ...
        create("ImageLabel", {
            ...
            Size = UDim2.new(0, size or 15, 0, size or 15),
            Position = UDim2.new(0.5, -(size or 15) / 2, 0.5, -(size or 15) / 2),
            BackgroundTransparency = 1,
            ImageColor3 = Theme.SubText,
            Parent = Btn,
        })
        Btn.MouseEnter:Connect(function()
            tween(Btn.ImageLabel, { ImageColor3 = Theme.Text }, 0.12)
        end)
        Btn.MouseLeave:Connect(function()
            tween(Btn.ImageLabel, { ImageColor3 = Theme.SubText }, 0.12)
        end)
        return Btn
    end

    ghostIconButton("search", -140)
    local MinBtn = ghostIconButton("minus", -104)
    ghostIconButton("maximize", -68)
    local CloseBtn = ghostIconButton("x", -32)
...
function Library:CreateTab(config)
    ...
    local TabButton = create("TextButton", {
        ...
        BackgroundColor3 = Theme.Element,
        BackgroundTransparency = self._first and 0.35 or 1,
        AutoButtonColor = false,
        Parent = self.TabList,
    }, { corner(7) })
    ...
    if icon then
        create("ImageLabel", {
            ...
            BackgroundTransparency = 1,
            ImageColor3 = self._first and Theme.Text or Theme.SubText,
            Parent = TabButton,
        })
    end
    ...
    local Label = create("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Position = UDim2.new(0, icon and 33 or 12, 0, 0),
    ...
    TabButton.MouseButton1Click:Connect(function()
        ...
        tween(TabButton, { BackgroundTransparency = 0.35 }, 0.15)
        Label.TextColor3 = Theme.Text
        if TabButton:FindFirstChildOfClass("ImageLabel") then
            TabButton:FindFirstChildOfClass("ImageLabel").ImageColor3 = Theme.Text
        end
        Page.Visible = true
    end)
...
function Library.CreateToggle(self, config)
    ...
    local Box = create("Frame", {
        ...
        BackgroundColor3 = state and Theme.Text or Theme.ElementHover,
        Parent = Holder,
    ...
    local function set(newState, fireCallback)
        state = newState
        tween(Box, { BackgroundColor3 = state and Theme.Text or Theme.ElementHover }, 0.15)
...
--// PARAGRAPH -----------------------------------------------------------------
function Library.CreateParagraph(self, config)
    config = config or {}
    local Holder = create("Frame", {
...
--// KEYBIND --------------------------------------------------------------------
function Library.CreateKeybind(self, config)
    config = config or {}
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        elseif not gpe and not listening and input.UserInputType == Enum.UserInputType.Keyboard
    ...
    return {
        Set = function(keyCode) current = keyCode; KeyButton.Text = keyCode.Name end,
        Get = function() return current end,
    }
end

--// COLOR PICKER ---------------------------------------------------------------
function Library.CreateColorPicker(self, config)
    config = config or {}
    ...
    local function channelRow(label, initial)
        ...
        local function updateColorFromChannels()
    ...
    local r, getR = channelRow("R", math.floor(color.R * 255))
    local g, getG = channelRow("G", math.floor(color.G * 255))
    local b, getB = channelRow("B", math.floor(color.B * 255))

    -- Recompute `color` whenever a channel changes by polling on drag release.
    -- Simpler approach: wrap updateColorFromChannels via closures below.
    for _, row in ipairs({ r, g, b }) do
        row.InputBegan:Connect(function() end) -- placeholder to keep row referenced
    end
    ...
    -- Hook refreshColor into each channel's drag release.
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            refreshColor()
        end
    ...
    Swatch.MouseButton1Click:Connect(function()
        open = not open
        tween(Holder, { Size = open and UDim2.new(1, 0, 0, 132) or UDim2.new(1, 0, 0, 34) }, 0.15)
    end)
